...
function ait_gridgalleryb( $params ) {
  ...
  if($category == '0'){
    $args = array( 'posts_per_page' => 500 , 'cat' => $category , 'post_type' => 'ait-grid-portfoliob', 'orderby' => 'menu_order', 'order' => 'ASC');
    ...
    foreach($categories as $cat){
      /* IMAGE RESOLVING */
      $args = array( 'post_type' => 'ait-grid-portfoliob', 'orderby' => 'menu_order', 'order' => 'ASC', 'posts_per_page' => 500, 'tax_query' => array( array( 'taxonomy' => 'ait-grid-portfoliob-category', 'field' => 'id', 'terms' => $cat) ) );
        ...
        if($image){
          switch($itemMetaValue['itemType']){
            case 'image':
              ...
              if($display == 'fancybox'){
                $result .= '<div class="tile" style="display: none" data-bigimg="'.$image[0].'" rel="ait-gridportfoliob"><div class="tileWrap" style="overflow: hidden">'.AitImageResizer::resize($image[0], array('w' => 250, 'h' => 250)).'</div><span class="gridgallery-icon"><a class="tileImage" href="'.$image[0].'">';
              } else {
            ...
            case 'video':
              if($display == 'fancybox'){
                $result .= '<div class="tile" style="display: none" data-bigimg="'.$image[0].'" rel="ait-gridportfoliob"><div class="tileWrap" style="overflow: hidden">'.AitImageResizer::resize($image[0], array('w' => 250, 'h' => 250)).'</div><span class="gridgallery-icon"><a class="tileImage" href="'.$itemMetaValue['videoLink'].'">';
              } else {
                $result .= '<div class="tile" style="display: none" data-bigimg="'.$image[0].'"><div class="tileWrap" style="overflow: hidden">'.AitImageResizer::resize($image[0], array('w' => 250, 'h' => 250)).'</div><span class="gridgallery-icon"><a class="tileImage" href="'.get_permalink( $item->ID ).'">';
              }
            break;
